...
function getNiveauRisque(prob: number): 'critique' | 'eleve' | 'moyen' | 'faible' | 'tres_faible' {
  if (prob > 60) return 'critique'; if (prob > 40) return 'eleve'
  if (prob > 20) return 'moyen'; if (prob > 5) return 'faible'; return 'tres_faible'
}

function getNiveauLabel(n: 'critique' | 'eleve' | 'moyen' | 'faible' | 'tres_faible'): string {
  switch (n) {
...
export default function BowTieAnalyzer({ profil, ecarts, surveillances }: Props) {
  ...
  return (
    <Card heading={<div className="flex items-center justify-between flex-wrap gap-3 w-full"><div className="flex items-center gap-2"><Shield className="w-5 h-5 text-role-primary" />Analyse Bow-Tie — Barrières de défense</div><select className="form-select py-1.5 text-sm" value={selectedDomaine} onChange={e => setSelectedDomaine(e.target.value)}>{DOMAINES.map(d => <option key={d} value={d}>{d}</option>)}</select></div>}>
      <div className="space-y-6">
        {!current ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={label} className={`rounded-xl p-3 border text-center ${cls}`}>
                  <div className="flex items-center justify-center gap-1 mb-1"><Icon className="w-4 h-4" /><span className="text-xs font-semibold text-foreground">{label}</span></div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3"><Shield className="w-4 h-4 text-success" /><span className="text-sm font-semibold text-foreground">Barrières préventives ({current.barrieresPreventives.length})</span></div>
                ...
                {current.barrieresPreventives.map(b => (
                  <div key={b.id} className={`p-3 rounded-lg border ${b.efficace ? 'border-success/30 bg-success-soft/30' : 'border-danger/30 bg-danger-soft/30'}`}>
                    <div className="flex items-start justify-between">
                      <div><p className="text-sm font-medium text-foreground">{b.nom}</p><p className="text-xs text-foreground">Dernier test: {b.dernierTest ? new Date(b.dernierTest).toLocaleDateString('fr-FR') : 'N/A'}</p></div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3"><Zap className="w-4 h-4 text-warning" /><span className="text-sm font-semibold text-foreground">Barrières correctives ({current.barrieresCorrectives.length})</span></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-xl bg-muted/20 border border-border">
                ...
                <span className={getRiskCls(current.probabiliteResiduelle)}>{getNiveauLabel(current.niveauRisqueResiduel)}</span>
              ...
              <div className="p-4 rounded-xl bg-success-soft/30 border border-success/20">
                ...
                <div className="mt-2 space-y-1 text-xs">
                  {profil.survival_metrics && <p className="flex items-center gap-1 text-foreground"><TrendingUp className="w-3 h-3 text-success" />Hazard 90j réduit: {Math.round(profil.survival_metrics.hazard90d * 40)}%</p>}
                  <p className="flex items-center gap-1 text-foreground"><Clock className="w-3 h-3 text-success" />Délai réponse: {profil.c2 > 50 ? 'Rapide' : 'À améliorer'}</p>
                  <p className="flex items-center gap-1 text-foreground"><Shield className="w-3 h-3 text-success" />Domaines protégés: {models.filter(m => m.probabiliteResiduelle < 30).length}/{models.length}</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-role-primary-soft/30 border border-role-primary/20">
                ...
                <div className="mt-2 space-y-1">
                  {[
                    ...
                    { c: 'C2', v: profil.c2, w: 25 },
                    { c: 'C3', v: profil.c3, w: 20 },
                    { c: 'C4', v: profil.c4, w: 20 },
                    { c: 'C5', v: profil.c5, w: 15 },
                  ].map(({ c, v, w }) => (
                  ...
                  <div className="border-t border-border pt-1 mt-1 flex justify-between text-xs">
                    <span className="font-semibold text-foreground">Impact global sur score</span>
            ...
            <div className="text-xs text-foreground pt-2 border-t border-border">
