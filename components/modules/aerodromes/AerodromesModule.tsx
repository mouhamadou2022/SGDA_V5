...
import { exporterListePDF, exporterFichePDF } from './aerodromesExport';
...
useEffect(() => setCurrentPage(1), [filters, searchTerm])
  ...
  // Exports PDF extraits : voir ./aerodromesExport.ts (mêmes données, mêmes toasts).
  const handleExportListePDF = () => exporterListePDF({
    aerodromes: filteredAerodromes, profilsRisque,
    setExporting: setExportingPdf, notify: toast,
  });

  const handleExportFichePDF = (aero: Aerodrome) => exporterFichePDF({
    aero,
    ctx: { profilsRisque, certifications, homologations, ecarts, surveillances },
