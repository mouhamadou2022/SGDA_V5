import { NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'

export async function POST(request: Request) {
  try {
    const { code } = await request.json()
    if (!code) {
      return NextResponse.json({ error: 'Code requis' }, { status: 400 })
    }

    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY
    if (!supabaseUrl || !serviceKey) {
      return NextResponse.json({ error: 'Configuration serveur manquante' }, { status: 500 })
    }

    const supabaseAdmin = createClient(supabaseUrl, serviceKey, {
      auth: { persistSession: false },
    })

    // 1. Chercher le code d'accès (service role bypasses RLS)
    const { data: codeData, error: codeError } = await supabaseAdmin
      .from('codes_acces')
      .select('*')
      .eq('code', code.toUpperCase())
      .eq('statut', 'actif')
      .single()

    if (codeError || !codeData) {
      return NextResponse.json({ error: "Code d'accès invalide ou expiré" }, { status: 401 })
    }
    if (codeData.expires_at && new Date(codeData.expires_at) < new Date()) {
      return NextResponse.json({ error: "Code d'accès expiré" }, { status: 401 })
    }

    // 2. Déterminer le rôle depuis le type de code
    let codeType = codeData.code_type
    if (!codeType) {
      const parts = code.toUpperCase().split('-')
      if (parts.length >= 2) codeType = parts[1]
    }
    const roleMap: Record<string, string> = { DG: 'dg_operator', FP: 'focal_operator', ST: 'staff_operator' }
    const expectedRole = roleMap[codeType || ''] || 'focal_operator'

    // 3. Déterminer l'email (collecté OBLIGATOIREMENT dans le formulaire de création du code)
    const identifiant = `${expectedRole}_${codeData.aerodrome_id}`.toLowerCase()
    const emailExploitant = codeData.email
    if (!emailExploitant) {
      return NextResponse.json({
        error: `Aucun email renseigné pour ce code d'accès. L'administrateur doit saisir un email dans le formulaire de création du code.`
      }, { status: 400 })
    }

    // 3b. Chercher l'utilisateur par email d'abord (quel que soit le statut) pour éviter les doublons
    let { data: utilisateur } = await supabaseAdmin
      .from('utilisateurs')
      .select('*')
      .eq('email', emailExploitant)
      .maybeSingle()

    if (!utilisateur) {
      // Fallback: chercher par aérodrome + rôle (si l'email a changé entre-temps)
      let { data: fallbackUser } = await supabaseAdmin
        .from('utilisateurs')
        .select('*')
        .eq('aerodrome_id', codeData.aerodrome_id)
        .eq('role', expectedRole)
        .maybeSingle()
      if (fallbackUser) utilisateur = fallbackUser
    }

    const nameMap: Record<string, { prenom: string; nom: string }> = {
      dg_operator:     { prenom: codeData.dg_prenom || 'DG',      nom: codeData.dg_nom || 'Exploitant' },
      focal_operator:  { prenom: codeData.focal_prenom || 'Focal',  nom: codeData.focal_nom || 'Exploitant' },
      staff_operator:  { prenom: codeData.staff_prenom || 'Staff',  nom: codeData.staff_nom || 'Exploitant' },
    }
    const names = nameMap[expectedRole] || { prenom: 'Exploitant', nom: 'Exploitant' }

    if (utilisateur) {
      // Mettre à jour l'utilisateur existant (réactiver le statut, forcer aerodrome_id)
      const { error: updateError } = await supabaseAdmin
        .from('utilisateurs')
        .update({
          prenom: names.prenom,
          nom: names.nom,
          statut: 'actif',
          aerodrome_id: codeData.aerodrome_id,
          role: expectedRole,
          password_temporaire: true,
          updated_at: new Date().toISOString(),
        })
        .eq('id', utilisateur.id)
      if (updateError) {
        console.error('[login-code] Erreur update utilisateur:', updateError)
      }
      // Re-fetch pour avoir les données à jour
      const { data: freshUser } = await supabaseAdmin
        .from('utilisateurs')
        .select('*')
        .eq('id', utilisateur.id)
        .single()
      if (freshUser) utilisateur = freshUser
    } else {
      // Créer un nouvel utilisateur
      const { data: newUser, error: createError } = await supabaseAdmin
        .from('utilisateurs')
        .insert({
          id: crypto.randomUUID(),
          aerodrome_id: codeData.aerodrome_id,
          email: emailExploitant,
          identifiant,
          nom: names.nom,
          prenom: names.prenom,
          role: expectedRole,
          statut: 'actif',
          password_temporaire: true,
          notifications_email: false,
          notifications_sms: false,
          force_pwd_change: false,
        })
        .select()
        .single()

      if (createError) {
        // Dernier recours: update si l'insert échoue (concurrence)
        const { data: retryUser } = await supabaseAdmin
          .from('utilisateurs')
          .update({
            prenom: names.prenom,
            nom: names.nom,
            statut: 'actif',
            aerodrome_id: codeData.aerodrome_id,
            role: expectedRole,
            password_temporaire: true,
          })
          .eq('email', emailExploitant)
          .select()
          .single()
        if (retryUser) {
          utilisateur = retryUser
        } else {
          return NextResponse.json({
            error: `Impossible de créer l'utilisateur ${expectedRole} : ${createError?.message || 'erreur inconnue'}`
          }, { status: 500 })
        }
      } else {
        utilisateur = newUser
      }
    }

    // 4. Créer/Mettre à jour l'utilisateur Supabase Auth (pas anonyme)
    const email = emailExploitant
    let authUser = null
    
    // Essayer de créer l'utilisateur Auth directement
    const defaultPassword = 'AnacimDNS@2026'
    const { data: newAuthUser, error: authCreateError } = await supabaseAdmin.auth.admin.createUser({
      email,
      password: defaultPassword,
      email_confirm: true,
      user_metadata: { role: expectedRole, aerodrome_id: codeData.aerodrome_id },
    })
    
    if (!authCreateError && newAuthUser?.user) {
      authUser = newAuthUser.user
    } else {
      // L'utilisateur Auth existe déjà → chercher via pagination
      console.warn('[login-code] createUser a échoué, recherche paginée:', authCreateError?.message)
      let page = 1
      const perPage = 1000
      while (true) {
        const { data: usersPage } = await supabaseAdmin.auth.admin.listUsers({ page, perPage })
        if (!usersPage?.users?.length) break
        authUser = usersPage.users.find(u => u.email === email)
        if (authUser) break
        if (usersPage.users.length < perPage) break
        page++
      }
      // Dernière chance: l'utilisateur db a déjà un auth_id (email changé dans le formulaire)
      if (!authUser && utilisateur.auth_id) {
        const { data: userById } = await supabaseAdmin.auth.admin.getUserById(utilisateur.auth_id)
        if (userById?.user) authUser = userById.user
      }
      if (!authUser) {
        return NextResponse.json({ error: 'Erreur création compte' }, { status: 500 })
      }
    }
    
    // Lier l'auth_id à l'utilisateur dans la table utilisateurs
    if (authUser && utilisateur.auth_id !== authUser.id) {
      await supabaseAdmin
        .from('utilisateurs')
        .update({ auth_id: authUser.id })
        .eq('id', utilisateur.id)
    }

    // 5. Incrémenter le compteur de connexions
    await supabaseAdmin
      .from('codes_acces')
      .update({
        last_login: new Date().toISOString(),
        nb_connexions: (codeData.nb_connexions ?? 0) + 1,
      })
      .eq('id', codeData.id)

    return NextResponse.json({
      user: {
        id: utilisateur.id,
        email: utilisateur.email ?? '',
        nom: utilisateur.nom,
        prenom: utilisateur.prenom,
        role: expectedRole,
        aerodrome_id: utilisateur.aerodrome_id,
        last_login: new Date().toISOString(),
      },
    })
  } catch (error: any) {
    console.error('[login-code] Erreur:', error)
    return NextResponse.json({ error: error.message || 'Erreur serveur' }, { status: 500 })
  }
}
